...
void LCD_PIN_Init(void){
	...
	//enable gpio clock, mode, output type
	
RCC->AHB2ENR = RCC->AHB2ENR | RCC_AHB2ENR_GPIOAEN | RCC_AHB2ENR_GPIOBEN | RCC_AHB2ENR_GPIOCEN |RCC_AHB2ENR_GPIODEN;
	
//RCC->AHB2ENR = RCC->AHB2ENR | RCC_AHB2ENR_GPIOAEN | RCC_AHB2ENR_GPIOBEN | RCC_AHB2ENR_GPIOCEN | RCC_AHB2ENR_GPIOEEN;


//set gpio mode register to alternate function mode for 6,7,8,9,10,15 which is 
//ORR with 1000 0000 0010 1010 1010 0000 0000 0000
//AND with 1011 1111 1110 1010 1010 1111 1111 1111
GPIOA->MODER &= 0xBFEAAFFF;
GPIOA->MODER |= 0x802AA000;

GPIOA->AFR[0] = 0xBB000000;//we were missing one B here
GPIOA->AFR[1] = 0xB0000BBB;


//set gpio mode register to alternate function mode for  0,1,4,5,9,12,13,14,15  which is 
//ORR with 1010 1010 0000 1000 0000 1010 0000 1010
//AND with 1010 1010 1111 1011 1111 1010 1111 1010
//AND LOW for Pins 0,1,4,5     1111 1111 1011 1011 1111 1111 1011 1011
//OR LOW for Pins 0,1,4,5	  1011 0000 0000 0000 0000 0000 0000 0000
//AND LOW for Pins 9,12,13,14,15	1111 1111 1011 1011 1111 1111 1011 1011
//OR LOW for Pins 9,12,13,14,15     1111 1111 1011 1011 1111 1111 1011 0000
...
//set gpio mode register to alternate function mode for 3,4,5,6,7,8 which is 
//ORR with 0000 0000 0000 0010 1010 1010 1000 0000
//AND with 1111 1111 1111 1110 1010 1010 1011 1111
GPIOC->MODER &= 0xFFFEAABF;
GPIOC->MODER |= 0x0002AA80;

//for afr[0] 1011 1011 1011 1011 1011 0000 0000 0000
//for afr[1] 0000 0000 0000 0000 0000 0000 0000 1011
GPIOC->AFR[0] = 0xBBBBB000;
GPIOC->AFR[1] = 0x0000000B;


//set gpio mode register to alternate function mode for 8,9,10,11,12,13,14,15 which is 
//ORR with 1010 1010 1010 1010 0000 0000 0000 0000
//AND with 1010 1010 1010 1010 1111 1111 1111 1111
GPIOD->MODER &= 0xAAAAFFFF;
GPIOD->MODER |= 0xAAAA0000;

//for afr[0] 0000 0000 0000 0000 0000 0000 0000 0000
//for afr[1] 1011 1011 1011 1011 1011 1011 1011 1011
...
void LCD_DisplayName(void){
	...
	LCD->RAM[3] |= CapLetterMap[14];

	LCD->RAM[4] |= CapLetterMap[4];

	LCD->RAM[5] |= CapLetterMap[15];
	//write to the update display request bit in the status register
	//wait for the udd to be true
}
...
{
		LCD->CR &= ~LCD_CR_LCDEN;
		//Must re enable but its not working now***************************************************************************
		//while ((LCD->SR & LCD_SR_FCRSR) != 0);		
		
		//bias[5:6] must be 10 and duty[2:4] must be 0011 mux[7] must be 0 vsel[1] must be 0 lcden[0] must be 1
		//0000 0000 0000 0000 0000 0000 0100 1100
		
		LCD->CR &= 0xFFFFFFB3;//2 to 3 for same reason as below
    LCD->CR |= 0x0000004C;//altered from D to C since that needs to happen after FCRSF of LCD_SR is set

		//Must re enable but its not working now***************************************************************************
		//while ((LCD->SR & LCD_SR_ENS) != 0);
		
		//LCD->FCR the CC[10:12] must be 111 and LCD->FCR PON[4:6] must be 111
		//0000 0000 0000 0000 0001 1100 0111 0000
		
		LCD->FCR &= 0xFFFFE38F;
		LCD->FCR |= 0x00001C70;

		
		while((LCD->SR & LCD_SR_FCRSR) ==0)//wait till FCRSF of LCD_SR# is set by bit 5 synched is value 1
    {// warning in while loop condition statement is favorable in this case
   	 (void)0;//noop
   	 //LCD->SR |=;
    }
    uint32_t WANT =0;
    WANT = LCD_CR_LCDEN;//enable the lcd by setting lcden bit of LCD_CR# is bit 0 value 1 is on
    LCD->CR |= WANT;
    
    while((LCD->SR & LCD_SR_ENS) == 0)//wait till lcd is enabled by checking the ENS bit of LCD_SR
    {
   	 (void)0;
    }
		//THIS IS A HUGE PROBLEM !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		//uint32_t temp = LCD->SR | LCD_SR_UDR;
		//LCD->SR = temp;
		//LCD->SR |= LCD_SR_UDR;
...
//    while(LCD->SR & LCD_SR_RDY == 0)//wait till the LCD booster is
//    {//ready by checking the RDY bit of LCD_SR
//   	 (void)0;
//    }
		
		LCD->SR |= LCD_SR_UDR;
    
    //might need to need to turn the LCD_SR_UDR to 1 here or wait till udr is set instead of SR_RDY
